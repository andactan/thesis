...
class VMPOModel(torch.nn.Module):
    def __init__(
        ...
        state_encoder_dim_hidden = 128,
        state_encoder_dim_output=64,
        num_experts=4,
        context_encoder_dim_input=768, # or 768 if roberta (100 GloVe)
        context_encoder_dim_hidden=256,
        context_encoder_dim_output=128

    ) -> None:
        super().__init__()

        self.state_size = 50  # memory state size (50 [expert encoder output] state + 100 context)
        ...
        self.policy_net = MlpModel(
            input_size=(state_encoder_dim_output + context_encoder_dim_output), hidden_sizes=256, output_size=2 * self.action_size
        )
        ...
        self.expert_encoder = ExpertEncoder(
            state_encoder_dim_input=self.size_dict['dim'],
    ...
    def forward(self, observation, prev_action, prev_reward, state):
        ...
        # sequence_len X batch ==> leading dimensions
        ...
        state_emb = self.expert_encoder(transformer_out, context_emb)
        
        if len(context_emb.shape) < 2:
            context_emb = context_emb.unsqueeze(dim=0)
        policy_in = torch.cat((state_emb, context_emb), dim=-1)
        
        policy_out = self.policy_net(policy_in).view(T * B, -1)
