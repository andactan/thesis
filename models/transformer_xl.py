...
class AttentionHead(torch.nn.Module):
    def __init__(self, input_dim, query_dim, key_dim, value_dim) -> None:
        super().__init__()
        self.q = torch.nn.Linear(input_dim, query_dim)
        self.k = torch.nn.Linear(input_dim, key_dim)
        self.v = torch.nn.Linear(input_dim, value_dim)
    ...
    def _scaled_dot_product(query, key, value):
