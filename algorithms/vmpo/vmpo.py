...
LossInputs = namedarraytuple(
    "LossInputs", ["agent_inputs", "action", "return_", "advantage", "valid", "old_dist_info"]
)
...
discrete_action_opt_info = ["alpha", "alpha_loss", "policy_kl", "entropy"]
continous_action_opt_info = [
    "alpha_mu",
    "alpha_mu_loss",
    "alpha_sigma",
    "alpha_sigma_loss",
...
OptInfo = namedtuple(
    "OptInfo", [*general_opt_info, *discrete_action_opt_info, *continous_action_opt_info]
)

SamplesToBuffer = namedarraytuple(
    "SamplesToBuffer", ["agent_inputs", "action", "return_", "advantage", "valid", "old_dist_info"]
)
...
class VMPO(RlAlgorithm):
    ...
    def __init__(
        self,
        discount=0.99,
        learning_rate=2e-4, # changed 1e-4 -> 2e-4
        T_target_steps=100,
        bootstrap_with_online_model=False,
        OptimCls=torch.optim.Adam, # changed
    ...
    def initialize(
        self, agent, n_itr, batch_spec, mid_batch_reset=False, examples=None, world_size=1, rank=0
    ):
        self.agent = agent
        self.alpha = torch.autograd.Variable(torch.ones(1) * self.initial_alpha, requires_grad=True)
        self.alpha_mu = torch.autograd.Variable(
            torch.ones(1) * self.initial_alpha_mu, requires_grad=True
        )
        self.alpha_sigma = torch.autograd.Variable(
            torch.ones(1) * self.initial_alpha_sigma, requires_grad=True
        )
    ...
    def process_returns(
        self, reward, done, value_prediction, action, dist_info, old_dist_info, opt_info
    ):
        ...
        loss, opt_info = self.loss(
            dist_info=dist_info[:-1],
    ...
    def optimize_agent(self, itr, samples=None, sampler_itr=None):
        ...
        init_rnn_states = buffer_to(
            samples.agent.agent_info.prev_rnn_state[0], device=self.agent.device
        )
    ...
    def loss(self, dist_info, value, action, return_, advantage, valid, old_dist_info, opt_info):
        ...
        advantage = advantage.clamp(
            min=-40, max=40
        )  # clamp due to numerical instability of exp function

        num_valid = valid.sum().type(torch.int32)
        k = num_valid // 2 if num_valid > 1 else num_valid
        top_advantages, top_advantages_indeces = torch.topk(
            (advantage.masked_fill(~valid.bool(), float("-inf"))).reshape(T * B), k
        )
        ...
        eta_loss = self.eta * self.epsilon_eta + self.eta * (
            log_advantage_sum - torch.log(0.5 * num_valid)
        )

        if self.discrete_actions:
            pi_loss, alpha_loss, opt_info = self.discrete_actions_loss(
                advantage_mask, psi, action, dist_info, old_dist_info, valid, opt_info
            )
            loss = pi_loss + value_loss + eta_loss + alpha_loss
        else:
            pi_loss, alpha_loss, opt_info = self.continuous_actions_loss(
                advantage_mask, psi, action, dist_info, old_dist_info, valid, opt_info
            )
            loss = pi_loss + value_loss + eta_loss + alpha_loss
    ...
    def discrete_actions_loss(
        self, advantage_mask, phi, action, dist_info, old_dist_info, valid, opt_info
    ):
        dist = self.agent.distribution
        pi_loss = -torch.sum(
            advantage_mask * (phi.detach() * dist.log_likelihood(action.contiguous(), dist_info))
        )
        policy_kl = dist.kl(old_dist_info, dist_info)
        alpha_loss = valid_mean(
            self.alpha * (self.epsilon_alpha - policy_kl.detach())
            + self.alpha.detach() * policy_kl,
            valid,
        )
    ...
    def beta_dist_loss(
        ...
        alpha_loss = valid_mean(
            self.alpha * (self.epsilon_alpha - kl.detach()) + self.alpha.detach() * kl, valid
        )
